...
| Traffic snapshot (live) | Traffic Images v2 (LTA DataMall) | `Traffic-Imagesv2` | JSON | **8 current live stills** (+ timestamp) | live, polled every 60 s |
...
| 4 | **Traffic snapshot camera locations** | ✅ | Every camera currently returned by the live Traffic Images API; 8 as of 22 Sep 2026 |
| 5 | **Layer toggles with live counts per layer** | ✅ | 3 switches with current source-backed counts, sub-type breakdown, totals line, and an “all layers hidden” notice |
...
| 9 | **“Don’t miss any, tell me how many are in use”** | ✅ | Every currently published Traffic Images record is plotted dynamically; marker count and live count both follow the runtime feed. |
...
| **Former Traffic Images cameras** | The official feed published about 90 cameras through 29 Jun 2026 and has published 8 since 30 Jun 2026; DataMall, data.gov.sg and OneMotoring agree on the current eight. | The layer follows the current API rather than presenting absent cameras as live or broken. |
| **LTA Road Camera inventory** | The separate 254-point dataset describes cameras installed to deter illegal parking and provides no public image URLs. | It is not included in the Traffic snapshot layer, avoiding the false impression that 254 feeds are broken. |
...
1. Every plotted point originates from a downloaded official file: the app parses the raw
   ...
   and the live feed in `rawTrafficImages()`.
2. The counts on screen are computed from the current payload at render time and match the
   official sources.
...
- To re-seed the bundled snapshot: `cp .cache/cameras.json src/data/cameras-seed.json`. The
  API prunes the illegal-parking inventory before it writes that cache, so the seed stays equal
  to the cameras the image feed actually publishes. The static bake
  (`scripts/build-static.mjs`) and the browser both re-apply the same prune for older seeds, so
  a stale payload can never put the inventory back on the map as "snapshot cameras".
- If the deployed site ever shows far more snapshot cameras than the feed serves (e.g. 262
  instead of 8), the deployed `data/cameras.json` is an older bake: re-run
  `npm run build:static` and push. Confirm with `curl -s <site>/data/cameras.json` that the
  `snapshot` layer `count` equals the live `Traffic-Imagesv2` camera count.
