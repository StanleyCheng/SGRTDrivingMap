...
| `npm run build:static` + local static host under `/SGRTDrivingMap/` | **14/14 checks pass**, including the live feed fetched browser-side from data.gov.sg |
| `node scripts/smoke.mjs https://stanleycheng.github.io/SGRTDrivingMap/` | **14/14 checks pass** on the deployed GitHub Pages site |
...
| | |
|---|---|
| Repository | https://github.com/StanleyCheng/SGRTDrivingMap (public) |
| Live site | **https://stanleycheng.github.io/SGRTDrivingMap/** |
...
| Base path | `/SGRTDrivingMap` (`next.config.ts`, `trailingSlash` on, `images.unoptimized`) |
...
(`node scripts/smoke.mjs https://stanleycheng.github.io/SGRTDrivingMap/`), including that the
